...
export default function BookingsReport() {
  ...
  // 🔥 reduce dashboard revenue on cancel
  const adjustRevenue = (bookingId) => {
    const cancelledBooking = bookings.find((b) => b.bookingId === bookingId);
    if (!cancelledBooking) return;

    const price = cancelledBooking.totalPrice || 0;
    let current = Number(localStorage.getItem("dashboardRevenue") || 0);
    let updated = current - price;

    if (updated < 0) updated = 0;
    localStorage.setItem("dashboardRevenue", updated);
  };
          ...
          <tbody>
            {bookings.map((b, idx) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-sm">{new Date(b.checkInDate).toLocaleDateString()}</td>
                <td className="px-4 py-3 text-sm">{new Date(b.checkOutDate).toLocaleDateString()}</td>
