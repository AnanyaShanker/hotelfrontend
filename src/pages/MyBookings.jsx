...
export default function MyBookings() {
    ...
    async (showLoader = true) => {
      ...
      try {
        ...
        try {
          ...
          if (response?.data && Array.isArray(response.data)) roomData = response.data;
          else if (Array.isArray(response)) roomData = response;
          else roomData = [];
        } catch {
          roomData = [];
        }
  ...
  useEffect(() => {
    if (location.state?.refresh) {
      setTimeout(() => fetchAllBookings(false), 500);
  ...
  const getAllBookings = useCallback(() => {
    ...
    if (filter === "FACILITY") combined = facilityList;
    else if (filter === "ROOM") combined = roomList;
    ...
    return (
      <PublicLayout>
        <div className="flex flex-col items-center justify-center min-h-screen">
          ...
          <div className="text-neutral-600 font-light text-sm uppercase tracking-widest">
            Loading Bookings...
          </div>
        </div>
      </PublicLayout>
    ...
    <PublicLayout>
      <div className="py-24 animate-fade-in">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
          ...
          <div className="bg-neutral-50 border border-neutral-200 p-6 text-center">
            <div className="text-3xl font-light text-neutral-900 mb-2">
              {[...facilityBookings, ...roomBookings].filter(
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-3">
                {["ALL", "FACILITY", "ROOM"].map((type) => (
                  ...
                  >
                    {type === "ALL" ? "All Bookings" : `${type} Bookings`}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="text-center py-20">
            ...
            <p className="text-neutral-500 font-light text-sm mb-8">
              {statusFilter !== "ALL"
                ...
                : "Start by booking a facility or room"}
            </p>
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                    <div className="flex-1">
                      ...
                      <div className="grid md:grid-cols-2 gap-3 text-sm">
                        ...
                        <div className="flex justify-between md:justify-start md:gap-2">
                          ...
                          <span
                            className={`font-light ${getPaymentBadge(
                    ...
                    <div className="flex flex-col items-end gap-4">
                      <span
                        className={`px-4 py-2 border text-xs uppercase tracking-widest font-light ${getStatusBadge(
                        ...
                        {booking.bookingStatus === "CONFIRMED" &&
                          booking.paymentStatus === "PENDING" && (
                            <button
                              onClick={() =>
                                navigate(
                                  `/payment/${booking.type.toLowerCase()}/${
                                    booking.facilityBookingId ||
                                    booking.bookingId
                                  }`
                                )
                              }
                              className="px-4 py-2 bg-neutral-800 text-white text-xs uppercase tracking-wid
est font-light hover:bg-neutral-900 transition"
                            >
                              Pay Now
                            </button>
                          )}
                        ...
                        {booking.bookingStatus === "COMPLETED" && (
                          <button
                            onClick={() =>
                              navigate(
                                `/feedback/${booking.displayId}?type=${booking.type.toLowerCase()}`
                              )
                            }
                            className="px-4 py-2 bg-blue-600 text-white text-xs uppercase tracking-widest font-light hover:bg-blue-700 transition"
                          >
                            Give Feedback
                          </button>
                        )}
                  ...
                  selectedBooking.bookingStatus === "CONFIRMED" && (
                    <button
                      onClick={() => {
                        setShowModal(false);
                        navigate(
                          `/payment/${selectedBooking.type.toLowerCase()}/${
                            selectedBooking.facilityBookingId ||
                            selectedBooking.bookingId
                          }`
                        );
              ...
              <div className="p-6 space-y-6">
                <div className="space-y-4">
                  <p className="text-neutral-700 font-light text-sm">
                    ...
                    {bookingToCancel.type.toLowerCase()} booking?
                  </p>

                  <div className="bg-amber-50 border border-amber-200 p-4 text-sm rounded">
                    <p className="text-amber-800 font-light">
                      ⚠️ <strong className="font-normal">Important:</strong> This
                      action cannot be undone.
                      {bookingToCancel.type === "ROOM" &&
